...
describe("rpc", () => {
    ...
    const cases = {
        ...
        "ten_resources": {
            ...
            registerResource: (ctx, dryrun, t, name, res) => {
                ...
                assert.strictEqual(name.substring(0, prefix.length), prefix, `Expected ${name} to be of the form ${prefix}N; missing prefix`);
            ...
            registerResource: (ctx, dryrun, t, name, res, deps) => {
                switch (name) {
                    ...
                    case "comp1":
                        ...
                        break;
                    case "res1":
                        assert.deepStrictEqual(deps, ["test:index:MyCustomResource::cust1", "test:index:MyCustomResource::cust2"]);
                        break;
                    default: throw new Error("Didn't check: " + name);
                }
                return { urn: makeUrn(t, name), id: undefined, props: undefined };
            ...
            registerResource: (ctx, dryrun, t, name, res, deps) => {
                switch (name) {
                    ...
                    case "cust3":
                        ...
                        break;
                    case "res1":
                        assert.deepStrictEqual(deps, ["test:index:MyCustomResource::cust1", "test:index:MyCustomResource::cust2", "test:index:MyCustomResource::cust3"]);
                        break;
                    default: throw new Error("Didn't check: " + name);
                }
                return { urn: makeUrn(t, name), id: undefined, props: undefined };
            ...
            registerResource: (ctx, dryrun, t, name, res, deps) => {
                switch (name) {
                    ...
                    case "res1":
                        ...
                        break;
                    case "res2":
                        assert.deepStrictEqual(deps, ["test:index:MyCustomResource::cust2", "test:index:MyCustomResource::cust3"]);
                        break;
                    case "res3":
                        assert.deepStrictEqual(deps, ["test:index:MyCustomResource::cust2"]);
                        break;
    ...
    for (const casename of Object.keys(cases)) {
        ...
        it(`run test: ${casename} (pwd=${opts.pwd},prog=${opts.program})`, util_1.asyncTest(() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            for (const dryrun of [true, false]) {
                ...
                (call, callback) => {
                    ...
                    if (req.getType() !== index_1.runtime.rootPulumiStackTypeName || opts.showRootResourceRegistration) {
                        if (opts.registerResource) {
                            ...
                            const { urn, id, props } = opts.registerResource(ctx, dryrun, t, name, res, deps, custom, protect, parent, provider, propertyDeps, ignoreChanges, version, importID, replaceOnChanges);
                            resp.setUrn(urn);
                            resp.setId(id);
                ...
                assert.strictEqual(regCnt, expectResourceCount, `Expected exactly ${expectResourceCount} resource registrations; got ${regCnt}`);
                if (opts.expectedLogs) {
...
function serveLanguageHostProcess(engineAddr) {
    ...
    let addrResolve;
    const addr = new Promise((resolve) => { addrResolve = resolve; });
